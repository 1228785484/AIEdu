      ...
      <table class="info-table">
        ...
        <tbody>
          <tr v-for="course in courseSelectionData" :key="course.id">
            ...
            <td>
              <el-button type="text" @click="navigateToCourse(course.courseId)">查看</el-button>
            </td>
            <td>
    ...
    setup() {
        ...
        try {
          ...
          const response = await fetch('http://localhost:8008/api/course/add', {
            ...
            body: JSON.stringify({
              ...
              title: newCourse.value.courseName,
      ...
      const confirmDeleteCourse = async () => {
        try {
          ...
          const response = await fetch(`http://localhost:8008/api/course/delete/${deleteCourse.value.courseId}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
          ...
          if (error.message !== 'cancel') {
            ...
            ElMessage.error(error.message || '删除课程失败，请稍后重试')
          }
        }
      }
      ...
      const navigateToCourse = (courseId) => {
        router.push(`/course/${courseId}/units`)
      }
